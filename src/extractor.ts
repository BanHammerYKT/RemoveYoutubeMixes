 ...
 */
class Extractor {
  subextractors: Set<(element: HTMLElement) => HTMLElement[]>;
   ...
   */
  extract(element: HTMLElement | Document, threshold: number): Video[] {
    const subextractors = Array.from(this.subextractors.values());
    const elements = [].concat(
      ...subextractors.map((subextractor) =>
        subextractor(element as HTMLElement)
      )
    );
    ...
    Logger.info(
      "Extractor.extract(): %d/%d Videos on the page were viewed.",
      unnecessary.length,
      unnecessary.length
    );
    return unnecessary;
   ...
   */
  insert(subextractor: (element: HTMLElement) => HTMLElement[]) {
   ...
   */
  remove(subextractor: (element: HTMLElement) => HTMLElement[]) {
 ...
 */
function extractExploreVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractGridVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractHistoryVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractHomeVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractPlaylistVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractRecommendedVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extractSearchVideos(element: HTMLElement): HTMLElement[] {
  return extract(
    element,
 ...
 */
function extract(element: HTMLElement, ...selectors: string[]): HTMLElement[] {
  return [].concat(
    ...selectors.map((selector) =>
      Array.from(element.querySelectorAll(`:scope ${selector}`))
    )
  );
}
